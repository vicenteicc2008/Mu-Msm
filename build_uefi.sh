...
function _help(){
	...
	echo "	--release <Build Mode>, -r <Build Mode>:    Release mode for building, 'DEBUG' is the default or use 'RELEASE' alternatively."
...
function _acpi(){
	...
	TARGET_DEVICE_VENDOR=$(grep TARGET_DEVICE_VENDOR ./config/$TARGET_DEVICE.conf | tr -d 'TARGET_DEVICE_VENDOR=' | tr -d '"')
...
cp ./Patches/Compile.patch ./Mu_Basecore/
cd Mu_Basecore
