...
function _help(){
	...
	echo
	echo "Build Project Mu UEFI for old Qualcomm Snapdragon Platforms."
	echo
	echo "Options:"
...
OPTS="$(getopt -o d:hfbcACDO:r:t:m:a: -l device:,help,release:,tool-chain:,memory: -n 'build_uefi.sh' -- "$@")"||exit 1
eval set -- "${OPTS}"
while true
do	case "${1}" in
